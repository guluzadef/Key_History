...
def search(request, id, str):
    context = {}
    if 'q' in request.GET:
        query = request.GET.get('q')
        return redirect('search', id=id, str=query)
    explore = Key.objects.filter(
        username_id=id and
        Q(type__icontains=str) |
        Q(host__icontains=str) |
        Q(port__icontains=str) |
        Q(user__icontains=str)
    )
...
def myfiles(request, id):
    ...
    if 'q' in request.GET:
        query = request.GET.get('q')
        return redirect('search', id=id, str=query)
    key = Key.objects.filter(username_id=id).all()
